...
Re_plot_stat <-
  Re_plot +
  geom_pointrange(data = filter(forcemeans, param == 'Re_body'),
                  aes(x = viscosity, y = estimate, ymin = estimate - std.error, ymax = estimate + std.error),
                  shape = 4, size = 0.5) +
  geom_line(data = filter(forcemeans, param == 'Re_body'),
                  aes(x = viscosity, y = estimate, group = 1)) +
  stat_pvalue_manual(data = filter(forcestats, param == 'Re_body'),
...
```{r}
ggsave('Re_plot.pdf', width = 2.17, height = 2)
```
