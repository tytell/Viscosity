...
```{r}
midlines <- read_hdf5(here('midlines_all_kinematics.h5')) %>%
...
amp_envelope_plot <-
  ...
  summarise(excmn = mean(abs(exc) / len.mm, na.rm = TRUE),
            excsd = sd(abs(exc) / len.mm, na.rm = TRUE),
            excse = excsd / sqrt(sum(!is.na(exc))),
            s = mean(s / len.mm, na.rm = TRUE)) %>%
  ggplot(aes(x = s, y = excmn, color = viscosity.cP, fill = viscosity.cP)) +
  ...
  coord_fixed(ratio = 5) +
  ylim(0, 0.072) +
  labs(x = 'Arc length along body (L)', y = 'Excursion (L)') +
  theme_light() +
